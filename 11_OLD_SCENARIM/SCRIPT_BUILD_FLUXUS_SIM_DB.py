...
LP_JSON = REPO / "06_DATA" / "DATA_DRE_BI_LP_MASTER_2025.json"
...
def main() -> None:
    ...
    # saldo carteira = soma spread LP corrigido para 0,8% (simulação 360)
    cur.execute(
        """
        UPDATE lps SET saldo_carteira_brl = (
            SELECT COALESCE(SUM(
                (CAST(gtv_brl AS REAL) * 0.008 * 0.428) -- LP Spread 0.342%
                - (CAST(gtv_brl AS REAL) * 0.0038)      -- IOF LP 0.38%
                - 1.0                                  -- Maint Share R$ 1.0
            ), 0)
    ...
    con.execute(
        ...
        WITH t AS (
          SELECT
            rematch_id,
            ROUND(
              CAST(gtv_brl AS REAL)
              + (CAST(gtv_brl AS REAL) * 0.008 * 0.428) -- LP Profit (0.342%)
              - (CAST(gtv_brl AS REAL) * 0.0038)      -- IOF LP (0.38%)
              - 1.0,                                  -- Maint Share (R$ 1.0)
              2
            ) AS vf,
            ROUND(
              COALESCE(
                LAG(
                  CAST(gtv_brl AS REAL)
                  + (CAST(gtv_brl AS REAL) * 0.008 * 0.428)
                  - (CAST(gtv_brl AS REAL) * 0.0038)
                  - 1.0
                ) OVER (
                  PARTITION BY lp_id
                  ORDER BY timestamp_remessa_iso ASC, rematch_id ASC
                ),
                CAST(gtv_brl AS REAL)
              ),
              2
